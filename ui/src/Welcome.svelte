  ...
  <EntryFrame title={!known ? 'Opening your space…' : creating ? 'Make yourself at home.' : 'Welcome back.'}
    ...
    {#if known}
      <form onsubmit={unlock} aria-busy={busy}>
        ...
        {#if mobile}<label class="remember"><input type="checkbox" aria-describedby="device-unlock-hint" bind:checked={rememberDevice} disabled={busy} />Remember on this device</label><p id="device-unlock-hint" class="hint">Uses secure device storage to reconnect after suspension. /lock removes the saved credential.</p>{/if}
