...
mod tests {
    ...
    fn decode_relay_card_accepts_private_and_rejects_public() {
        ...
        assert!(
            crate::client::decode_relay_card(&URL_SAFE_NO_PAD.encode(public.encode())).is_err()
        );
        assert!(crate::client::decode_relay_card(&valid_private_card(now_unix)).is_ok());
        assert!(
            crate::client::decode_contact_card(&URL_SAFE_NO_PAD.encode(public.encode())).is_ok()
        );
    }
}
